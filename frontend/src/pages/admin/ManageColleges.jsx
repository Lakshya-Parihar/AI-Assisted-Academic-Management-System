...
const ManageColleges = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await adminApi.delete(`/colleges/${id}`);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await adminApi.put(`/colleges/${selectedCollege.id}`, selectedCollege);
      setColleges((prev) =>
  ...
  const handleOpenModal = (college, mode) => {
  ...
  const filteredColleges = colleges.filter((c) => {
    ...
    return `${c.college_name || ""} ${c.college_code || ""} ${c.city || ""} ${c.state || ""} ${c.clg_address || ""} ${c.university_name || ""} ${c.status || ""}`
      .toLowerCase()
      .includes(term);
  });
                ...
                <tr>
                  <td colSpan="5" style={{ textAlign: "center", padding: "4rem 2rem" }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: "#64748b" }}>
                      ...
                      <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'white' }}>No Colleges Found</div>
                  ...
                  <tr key={c.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 700, color: 'white' }}>{c.college_name}</div>
                          {c.city && <div style={{ fontSize: '0.75rem', color: '#94a3b8' }}>{c.city}, {c.state}</div>}
                        </div>
                      </div>
                    </td>
                    <td style={{ fontFamily: "monospace", color: "#818cf8", fontWeight: 600 }}>{c.college_code}</td>
                    <td style={{ color: '#cbd5e1' }}>{c.university_name || "N/A"}</td>
                    <td>
                      <span style={{
                    ...
                    <td>
                      <div style={{ display: "flex", gap: "0.5rem", justifyContent: 'flex-end' }}>
                        ...
                        <button className="action-btn" onClick={() => handleOpenModal(c, "edit")} title="Edit College">
                          <Edit3 size={18} />
                        </button>
                        <button className="action-btn danger" onClick={() => handleDelete(c.id)} title="Delete Record">
                          <Trash2 size={18} />
                        </button>
                      </div>
                    </td>
                ...
                {modalMode === "view" ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginTop: "0.75rem", marginBottom: "3rem" }}>
                      ...
                      <span style={{
                        background: selectedCollege.status === 'Active' ? "rgba(16, 185, 129, 0.15)" : "rgba(245, 158, 11, 0.15)",
                        color: selectedCollege.status === 'Active' ? "#10b981" : "#f59e0b",
                        border: `1px solid ${selectedCollege.status === 'Active' ? 'rgba(16,185,129,0.3)' : 'rgba(245,158,11,0.3)'}`,
                    ...
                    <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                      ...
                      <div className="info-box">
                        <h4 className="info-box-title">Affiliation & Details</h4>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                          {[
                            ["University Name", selectedCollege.university_name],
                            ["Established Year", selectedCollege.established_year],
                          ].map(([label, val]) => (
                            <div key={label}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                          <div>
                            ...
                            <div style={{ color: 'white', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <Mail size={16} color="#6366f1" /> {selectedCollege.hod_email || "-"}
                            </div>
                          </div>
                          <div>
                            <div style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 700, marginBottom: '4px' }}>Phone Number</div>
                            <div style={{ color: 'white', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <Phone size={16} color="#6366f1" /> {selectedCollege.hod_phone || "-"}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      {/* Location Box */}
                      <div className="info-box">
                        <h4 className="info-box-title">Location Address</h4>
                        <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                          <MapPin size={20} color="#6366f1" style={{ marginTop: '2px' }} />
                          <div style={{ color: 'white', fontWeight: 600, fontSize: '1rem', lineHeight: '1.5' }}>
                            {selectedCollege.clg_address || "No address provided."}
                          </div>
                        </div>
                      </div>
                  ...
                  <>
                    <div style={{ textAlign: "center", marginBottom: "3rem" }}>
                      ...
                      <p style={{ color: "#94a3b8", marginTop: "0.5rem" }}>
                        Update institution details and affiliation records
                      </p>
                    </div>
                    ...
                    <form onSubmit={handleUpdate}>
                      <div className="details-grid">
                        ...
                        <div className="input-group">
                          ...
                          <input
                            name="college_code"
                            value={selectedCollege.college_code}
                            onChange={handleInputChange}
                          />
                        </div>
                        <div className="input-group">
                          ...
                          <input
                            ...
                            value={selectedCollege.university_name || ""}
                            onChange={handleInputChange}
                          />
                        </div>
                        <div className="input-group">
                          ...
                          <input
                            ...
                            value={selectedCollege.established_year || ""}
                            onChange={handleInputChange}
                          />
                        </div>
                        <div className="input-group">
                          ...
                          <input
                            type="email"
                            name="hod_email"
                            value={selectedCollege.hod_email || ""}
                            onChange={handleInputChange}
                          />
                        </div>
                        <div className="input-group">
                          ...
                          <input
                            name="hod_phone"
                            value={selectedCollege.hod_phone || ""}
                            onChange={handleInputChange}
                          />
                        </div>
                        <div className="input-group">
                          ...
                          <select
                            name="status"
                            value={selectedCollege.status || "Active"}
                            onChange={handleInputChange}
                          >
                        ...
                        <div className="input-group full-width" style={{ gridColumn: "span 2" }}>
                          ...
                          <textarea
                            rows="2"
                            name="clg_address"
                            value={selectedCollege.clg_address || ""}
                            onChange={handleInputChange}
